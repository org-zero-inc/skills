#!/usr/bin/env python3
"""从 Pexels 搜索并下载高质量免费图片（无需 API Key，使用 curl_cffi 绕过反爬）。"""
...
def download_photo(photo, output_dir, index):
    ...
    for attempt, url in enumerate(urls_to_try):
        try:
            ...
            print(f"  -> #{index + 1}: {os.path.basename(filepath)} ({size_kb:.0f} KB) [pexels]")
            return filepath
        except Exception as e:
            if attempt < len(urls_to_try) - 1:
                continue
            print(f"  !! #{index + 1}: 失败 - {e} [pexels]", file=sys.stderr)
            return None


def main():
    parser = argparse.ArgumentParser(description="从 Pexels 搜索并下载高质量免费图片")
    parser.add_argument("keyword", help="搜索关键词")
    parser.add_argument("-n", "--count", type=int, default=5, help="下载数量（默认 5，最大 50）")
    parser.add_argument("-o", "--output", default="", help="保存目录（默认 ./downloaded_images_pexels/<keyword>）")
    parser.add_argument("--delay", type=float, default=0.5, help="下载间隔秒数（默认 0.5）")
    args = parser.parse_args()

    keyword = args.keyword
    count = min(max(args.count, 1), 50)
    output_dir = args.output or os.path.join(".", "downloaded_images_pexels", sanitize_filename(keyword))

    os.makedirs(output_dir, exist_ok=True)

    print(f"[search] 关键词: {keyword} | 数量: {count}")
    print(f"[output] {os.path.abspath(output_dir)}")
    print()

    print(f"[pexels] 搜索 {count} 张...")
    all_photos = search_pexels(keyword, count)
    print(f"[pexels] 找到 {len(all_photos)} 张")
    ...
    if results["success"]:
        ...
        for item in results["success"]:
            print(f"  - {os.path.basename(item['file'])} (pexels) {item['page_url']}")
