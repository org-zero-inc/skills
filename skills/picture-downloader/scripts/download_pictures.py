#!/usr/bin/env python3
"""从 Unsplash 和 Pexels 搜索并下载图片（无需 API Key，使用 curl_cffi 绕过反爬）。"""
...
def search_pexels(keyword, count):
    ...
    try:
        ...
        print(f"  Pexels 搜索失败: {e}", file=sys.stderr)
        return []

    img_matches = re.findall(
        r'https://images\.pexels\.com/photos/(\d+)/([^\s"\'<>]+?\.(?:jpeg|jpg|png|webp))',
        html,
    )

    seen = {}
    for pid, path in img_matches:
        if pid not in seen:
            seen[pid] = path

    fallback_ids = re.findall(r'https://images\.pexels\.com/photos/(\d+)/', html)
    for pid in fallback_ids:
        if pid not in seen:
            seen[pid] = None

    photos = []
    for pid, path in list(seen.items())[:count]:
        if path:
            url = f"https://images.pexels.com/photos/{pid}/{path}"
            url = re.sub(r'[?&](?:cs|auto|w|h|fit|crop)=[^&"\'<>]+', '', url)
        else:
            url = f"https://images.pexels.com/photos/{pid}/pexels-photo-{pid}.jpeg"
        photos.append({
            "id": pid,
            "url": url,
            "fallback_url": f"https://images.pexels.com/photos/{pid}/pexels-photo-{pid}.jpeg?auto=compress&cs=tinysrgb&w=1260",
            "description": keyword,
            "photographer": "unknown",
            "width": 0,
            "height": 0,
...
def download_photo(photo, output_dir, index):
    ...
    for attempt, url in enumerate(urls_to_try):
        try:
            ...
            print(f"  -> #{index + 1}: {os.path.basename(filepath)} ({size_kb:.0f} KB) [{photo['source']}]")
            return filepath
        except Exception as e:
            if attempt < len(urls_to_try) - 1:
                continue
            print(f"  !! #{index + 1}: 失败 - {e} [{photo['source']}]", file=sys.stderr)
            return None


def main():
    parser = argparse.ArgumentParser(description="从 Unsplash/Pexels 搜索并下载图片")
    parser.add_argument("keyword", help="搜索关键词")
    parser.add_argument("-n", "--count", type=int, default=5, help="下载数量（默认 5）")
    parser.add_argument("-s", "--source", choices=["unsplash", "pexels", "both"], default="both", help="图片来源（默认 both）")
    parser.add_argument("-o", "--output", default="", help="保存目录（默认 ./downloaded_images/<keyword>）")
    parser.add_argument("--size", choices=["raw", "full", "regular", "small", "thumb"], default="raw", help="Unsplash 图片尺寸（默认 raw）")
    parser.add_argument("--delay", type=float, default=0.5, help="下载间隔秒数（默认 0.5）")
    args = parser.parse_args()

    keyword = args.keyword
    count = min(max(args.count, 1), 50)
    output_dir = args.output or os.path.join(".", "downloaded_images", sanitize_filename(keyword))

    os.makedirs(output_dir, exist_ok=True)

    print(f"[search] 关键词: {keyword} | 数量: {count} | 来源: {args.source} | 尺寸: {args.size}")
    ...
    pexels_count = 0
    if args.source == "both":
        unsplash_count = (count + 1) // 2
        pexels_count = count - unsplash_count
    ...
    if unsplash_count > 0:
        print(f"[unsplash] 搜索 {unsplash_count} 张...")
        photos = search_unsplash(keyword, unsplash_count, size=args.size)
        all_photos.extend(photos)
        print(f"[unsplash] 找到 {len(photos)} 张")

    if pexels_count > 0:
        print(f"[pexels] 搜索 {pexels_count} 张...")
        photos = search_pexels(keyword, pexels_count)
        all_photos.extend(photos)
    ...
    if results["success"]:
        ...
        for item in results["success"]:
            print(f"  - {os.path.basename(item['file'])} ({item['source']}) {item['page_url']}")
